    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let index = 0; index < TotalRegistros; index++) {
                ...
                let Arreglo = [id, moment(FormatoFechaInicio).format("DD/MM/YYYY HH:mm"), Inspector, Operador, Tipo, OT, CantidadOT, Serie, Maquina, moment(FormatoFechaFin).format("DD/MM/YYYY HH:mm"), CantidadConforme, CantidadNoConforme, CantidadRetrabajo, CantidadAjuste, Notas, Minutos];
                // inserta una fila al final de la tabla
                var newRow = Tabla.insertRow(Tabla.rows.length);
...
function ExcelReporte() {
    ...
    let encabezado = ["Fecha registro", "Hora", "Inspector", "Operador", "Tipo", "OT", "Cantidad OT", "Serie", "Maquina", "Fecha fin", "Hora", "Conforme", "No Conforme", "Retrabajo", "Ajuste", "Notas", "Minutos"];
    sheet_1_data.push(encabezado)

    for (var j = 1; j <= total - 1; j++) { //filas
        //var Folio = tabla.rows[j].cells[0].childNodes[0].nodeValue;
        var FechaReg = tabla.rows[j].cells[1].childNodes[0].nodeValue;
        ...
        var Fila = [FechaRegistro, horaInicio, Inspector, Operador, Tipo, OT, CantidadOT, Serie, Maquina, FechaFin, horaFin, Conforme, NoConforme, Retrabajo, Ajuste, Notas, Minutos]
